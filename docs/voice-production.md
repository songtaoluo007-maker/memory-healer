...
2. 候选留在 `.codex-run/voice-renders`，人工审核后由脚本校验并事务式晋升；
...
晋升命令会先复制到 staging、把候选标为批准、执行完整媒体/manifest 校验，再
替换运行时文件和 manifest。复制、删除或 manifest 替换失败时会恢复上一套资产：
...
成功后应看到七条批准资产且没有 stale file。提交前只纳入经审核的
`backend/data/voice_public/fixed/*.opus` 与 `backend/data/voice_assets.json`；
`.codex-run` 中的 raw、cache 和候选不是发布资产。
...
Compose 中 `backend/data/voice_public/fixed` 只读挂载到容器，
`voice-cache` 与 `tts-cache` 是独立可写卷。不要把 seed、模型或权重目录加入
Compose。
...
仅当已有受控的外部 HTTPS 服务时才设置 `VOICE_PRIMARY_ENABLED=true`。为兼容
已有 adapter，远程 URL/token 仍使用上面的 `COSYVOICE_*` 环境变量名；配置会
拒绝空 token、HTTP 和 loopback URL。
...
$env:COSYVOICE_BASE_URL = "https://voice-primary.invalid"
$env:COSYVOICE_BRIDGE_TOKEN = "failure-drill-only"
...
Remove-Item Env:COSYVOICE_BASE_URL -ErrorAction SilentlyContinue
Remove-Item Env:COSYVOICE_BRIDGE_TOKEN -ErrorAction SilentlyContinue
```
...
每次运行时合成只写一个 JSON `voice_synthesis` 事件，例如：

```json
{"cache_hit":false,"event":"voice_synthesis","fallback":true,"latency_ms":25.0,"provider":"edge","silent_degradation":false}
```

聚合以下字段：

- `provider`：`fixed`、`cosyvoice`（远程兼容标识）、`edge` 或 `silent`；
- `cache_hit`：是否命中对应运行时缓存；
